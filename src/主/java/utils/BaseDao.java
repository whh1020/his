...
public class BaseDao {
    ...
    //需要生成mapper文件的sql语句
    private static String sql = "select dvr.vr_id,bpi.patient_name,bu.cname,dvr.visit_date,dvr.symptom,dvr.advice,dvr.times,\n" +
            "case dvr.status when 1 then '已就诊' when 2 then '已缴费' when 3 then '已领药' end as statusdesc\n" +
            "from doctor_visit_record dvr join base_patient_info bpi on dvr.patient_id=bpi.patient_id\n" +
            "\t\t\t     join base_user bu on dvr.doctor_id=bu.user_id";
    //主表
    private static String mainTable = "doctor_visit_record";
